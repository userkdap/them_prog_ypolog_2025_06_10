...
try:
    ...
    with open(INPUTFILENAME, 'r', encoding="utf-8") as inputfile:
        for ypopsifios in range(1,YPOPSIFIOI+1):
            kod_ypops = inputfile.readline().strip('\n').strip('\ufeff')
            print("Κωδικός υποψηφίου {}: {}".format(ypopsifios, kod_ypops))
            KOD.append(kod_ypops)
            synol_vathm = 0
            for erotisi in range(0,EROTISEIS):
                apantisi = inputfile.readline().strip('\n').strip('\ufeff')
                print("Απάντηση στην ερώτηση {} [a, b, c, d]: {}".format(erotisi+1, apantisi))
